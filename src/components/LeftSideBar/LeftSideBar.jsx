...
const LeftSideBar = () => {
      ...
      if (snapshot.exists()) {
        const fetchChatsWithUserData = async () => {
          try {
            ...
            const sortedChats = chatsWithUserData.sort((a, b) => b.updatedAt - a.updatedAt);
            setExistingChats(sortedChats);
          } catch (error) {
            console.error("Error processing chats:", error);
          }
  ...
  const inputHandler = async (e) => {
    ...
    try {
      const usersRef = collection(db, "users");
      const queries = [
        query(usersRef, where("username", ">=", searchTerm), where("username", "<=", searchTerm + "\uf8ff")),
        query(usersRef, where("email", "==", searchTerm)),
      ];
      ...
      <div className="ls-list">
        {searchQuery.length > 0 ? (
          isSearching ? (
            ...
            searchResults.map((user) => (
              <div onClick={() => addChat(user)} className="friends add-user" key={user.id}>
                <img
                  src={user.avatar || assets.profile_img}
                  alt={user.name || user.username}
                  onError={(e) => (e.target.src = assets.profile_img)}
          ...
          existingChats.map((chat) => (
            ...
            >
              ...
              />
              <div className="friend-info">
                <p className="friend-name">{chat.userData?.name || chat.userData?.username}</p>
